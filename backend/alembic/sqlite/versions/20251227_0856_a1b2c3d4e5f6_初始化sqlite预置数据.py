...
def upgrade() -> None:
    ...
    print(f"[OK] SQLite: 已插入 {len(relationship_types_data)} 条关系类型数据")
...
4. 保持轻松愉快的基调""",
    ...
    print(f"[OK] SQLite: 已插入 {len(writing_styles_data)} 条全局写作风格预设")
...
def downgrade() -> None:
    ...
    print("[OK] SQLite: 已删除全局写作风格预设")
    
    # 删除关系类型
    op.execute("DELETE FROM relationship_types")
    print("[OK] SQLite: 已删除关系类型数据")