...
@router.post("/{chapter_id}/generate-title", summary="AI生成章节标题")
async def generate_chapter_title(
    ...
    # 构建提示词
    prompt = f"""你是一位专业的小说编辑，请根据以下章节内容，生成一个简洁、吸引人的章节标题。

小说名称：《{project.title}》
章节序号：第{chapter.chapter_number}章
章节内容：
{chapter.content[:2000]}  # 限制内容长度
...
请直接输出标题，不要有任何其他说明。"""
    ...
    try:
        ...
        # 从响应中提取内容
        generated_title = response.get('content', '').strip()
        ...
        if not generated_title:
            raise HTTPException(status_code=500, detail="AI生成的标题为空")
