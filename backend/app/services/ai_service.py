...
class AIService:
    ...
    def __init__(
        ...
        if openai_key:
            # 确定Base URL：优先使用传入的api_base_url，如果为空则使用应用配置，最后使用默认OpenAI URL
            if api_provider == "openai" and api_base_url:
                base_url = api_base_url
            elif api_provider == "openai" and not api_base_url:
                # 用户指定openai提供商但没有提供base_url，使用应用配置或默认
                base_url = app_settings.openai_base_url or "https://api.openai.com/v1"
            else:
                # 使用其他提供商但需要OpenAI客户端（如DeepSeek），使用应用配置
                base_url = app_settings.openai_base_url or "https://api.openai.com/v1"
            
            logger.info(f"🔧 创建OpenAI客户端: base_url={base_url}, openai_key_prefix={openai_key[:10] if openai_key else 'None'}...")
            client = OpenAIClient(openai_key, base_url, self.config)
