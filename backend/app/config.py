...
# 获取backend目录(从backend/app/config.py向上两级)
BACKEND_ROOT = Path(__file__).parent.parent
# 项目根目录（backend的父目录）
PROJECT_ROOT = BACKEND_ROOT.parent
# 数据目录：SQLite数据库文件存放位置（项目根目录下的data文件夹）
...
if env_database_url:
    ...
    # 默认使用SQLite绝对路径
    db_file_path = os.path.abspath(DATA_DIR / "mumuai_novel.db")
    DATABASE_URL = f"sqlite+aiosqlite:///{db_file_path}"
    config_logger.debug(f"使用默认SQLite数据库路径: {db_file_path}")

# 检测数据库类型
if 'sqlite' in DATABASE_URL.lower():
    config_logger.debug(f"数据库类型: SQLite")
else:
